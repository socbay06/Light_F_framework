...
float get_lf_data(int x, int y, int s, int t,int c){
  //printf("get_lf_data for %d %d %d %d %d\n",x,y,s,t,c);
  return data[c*W*H*S*T + x*H*S*T + y*S*T + s*T + t];
}
...
void handleComputeDisplacements()
{
  ...
  float gamma;
  int center = floor((S-1)/2);
  float gapfx,gapfy;
  int count;
  int gapx,gapy;
  gamma=1.0*(1.0 - 1.0/g_e_sigma);
  int cout=0;

  printf(" start compute ref\n");
  for(int i =g_bx; i<g_nx+g_bx;i++)
    for(int j=g_by; j<g_ny+g_by;j++){
        count=0;
        for(int kx=-g_m_alpha; kx<=g_m_alpha;kx++){
          gapfx = kx*(gamma);
          gapx=floor(gapfx);
          for(int ky=-g_m_alpha; ky<=g_m_alpha;ky++){
            gapfy = ky*(gamma);
            gapy=floor(gapfy);
            if((i+gapx)>=g_bx && (i+gapx)<(g_nx+g_bx) && (j+gapy)>=g_by && (j+gapy)<(g_ny+g_by)){
              g_disp[i][j][0]+=get_lf_data(i+gapx-g_bx,j+gapy-g_by,center+kx,center+ky,0);
              g_disp[i][j][1]+=get_lf_data(i+gapx-g_bx,j+gapy-g_by,center+kx,center+ky,1);
              g_disp[i][j][2]+=get_lf_data(i+gapx-g_bx,j+gapy-g_by,center+kx,center+ky,2);
              count++;
          }
        }
      }
...
void handleDraw()
{
        ...
        //
        copy_refocus_image(g_f1,g_disp,g_nx,g_ny,g_bx,g_by,
                                       (float)0.0,g_g_max_disp,g_p6);
