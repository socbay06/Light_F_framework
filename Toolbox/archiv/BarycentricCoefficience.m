...
if  and((round(c/Scale)-c/Scale)>=0,(round(r/Scale)-r/Scale)>0)
    A(2)=A(2)-1;
    A(7)=A(7)+1;
    %4 5 8
    A=inv(A);
...
elseif and((round(c/Scale)-c/Scale)>0,(round(r/Scale)-r/Scale)<=0)
    A(5)=A(5)+1;
    A(7)=A(7)+1;
    %5 6 8
    A=inv(A);
...
else and((round(c/Scale)-c/Scale)==0,(round(r/Scale)-r/Scale)==0)
    %5
    A=inv(A);
    f = A*transpose([c/Scale r/Scale 1]);
